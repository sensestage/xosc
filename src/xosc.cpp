...
{ 
  ...
  // tag, client ip, port
  lo_address clientaddr = lo_address_create_from( &argv[1]->s, argv[2]->i  );
  XOscClient * myclient = server->clientExists( argv[2]->i, clientaddr );
  if ( myclient == NULL ){
    myclient = server->createNewClient( argv[2]->i, clientaddr );
  }
